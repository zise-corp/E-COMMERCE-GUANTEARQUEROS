...
export function BrandsManager({ rows, openNew = false }: { rows: AdminBrandRow[]; openNew?: boolean }) {
  ...
  const [form, setForm] = useState<Editing | null>(() => openNew ? { id: 0, name: "", slug: "", accentHex: null, logoPath: null, logoFileId: null, active: true, isOwnBrand: false } : null);
  ...
  function save() {
    ...
    startTransition(async () => {
      const result = await saveBrandAction({ name: form.name, accentHex: form.accentHex, logoPath: form.logoPath, logoFileId: form.logoFileId, active: form.active, isOwnBrand: form.isOwnBrand }, form.id || undefined);
  ...
  return <>
    <div className="border border-ink-700 bg-ink-850">
      <div className="hidden border-b border-ink-700 px-5 py-3 text-[10.5px] uppercase tracking-[0.16em] text-content-dim lg:grid lg:grid-cols-[52px_1.6fr_1fr_100px_90px_80px] lg:gap-3.5"><span /><span>Marca</span><span>Tipo</span><span>Productos</span><span>Estado</span><span /></div>
      ...
      {rows.map((row) => <div key={row.id} className="grid items-center gap-3.5 border-b border-line-soft px-5 py-3 lg:grid-cols-[52px_1.6fr_1fr_100px_90px_80px]">
        <div className="relative h-10 w-10 overflow-hidden bg-ink-950">{row.logoPath ? <Image src={imageKitUrl(row.logoPath, "thumb")} alt="" fill sizes="40px" className="object-contain" /> : <span className="flex h-full items-center justify-center font-display text-sm text-content-faint">{row.name.slice(0, 2).toUpperCase()}</span>}</div>
...
function BrandModal({ form, pending, error, onChange, onClose, onSave }: { form: Editing | null; pending: boolean; error: string | null; onChange: (next: Editing) => void; onClose: () => void; onSave: () => void }) {
  ...
  if (!form) return null;
  const logo: ProductImageValue[] = form.logoPath ? [{ publicId: form.logoPath, fileId: form.logoFileId, alt: form.name }] : [];
  return <Portal><div className="fixed inset-0 z-[70] overflow-y-auto bg-[#040404]/[0.78] p-5 sm:p-10"><div ref={ref} role="dialog" aria-modal="true" aria-label="Marca" tabIndex={-1} className="mx-auto w-full max-w-[680px] border border-line-strong bg-ink-850 animate-rise outline-none">
    ...
    <div className="grid gap-5 p-6 md:grid-cols-[1fr_0.9fr]"><div className="flex flex-col gap-3.5"><Input label="Nombre de la marca" value={form.name} className="bg-[#0E0E0D]" onChange={(event) => onChange({ ...form, name: event.target.value })} /><Input label="Color de acento" value={form.accentHex ?? ""} placeholder="#FA2A00" hint="Opcional, en formato hexadecimal." className="bg-[#0E0E0D]" onChange={(event) => onChange({ ...form, accentHex: event.target.value || null })} /><div className="border border-ink-700 bg-[#0E0E0D] p-3.5"><Toggle checked={form.active} label="Visible en la tienda" onChange={(active) => onChange({ ...form, active })} /></div><div className="border border-drei-line/40 bg-drei/[0.08] p-3.5"><Toggle checked={form.isOwnBrand} label="Es la marca propia" onChange={(isOwnBrand) => onChange({ ...form, isOwnBrand })} /><p className="mt-2 text-[11px] text-content-dim">Solo una marca puede estar marcada como propia.</p></div></div><ImageKitDropzone slug={form.slug || slugify(form.name)} value={logo} onChange={(next) => { const image = next[0]; onChange({ ...form, logoPath: image?.publicId ?? null, logoFileId: image?.fileId ?? null }); }} folder="/guantearqueros/marcas" maxImages={1} label="Logo de marca · ImageKit" assetTag="marca" /></div>
